...
6. **Affirmative Single-Turn Boundary & Anti-Scriptwriting Stop Tokens (`FEAT-489` in `src/nodes/loader.py` & `src/logic/cognitive_hub.py`)**:
   - *Positive Prompt Boundary:* Replaced negative simulation prohibitions with affirmative single-turn instruction: *"Single-Turn Boundary: Speak strictly for your own active turn. Deliver your 3-4 dense technical bullet points directly, then yield the floor."*
