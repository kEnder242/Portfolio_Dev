...
**Parent Framework:** BKM-020 (High-Fidelity Sprint Documentation), BKM-046 (Fast-Path DNA Retrieval), BKM-024 (Live Verification), FEAT-564 (Writer Studio & LaTeX Pipeline), FEAT-580 (Paper Dataset Architecture), FEAT-581A (Organization Tree View), FEAT-581B (Single-Card Review Inspector), FEAT-581C (Synthesis Reflow & Revision Carousel), FEAT-582 (Cross-Collection DNA Citations), FEAT-583 (Cascade Synthesis Engine), FEAT-584 (Paper Voice Control).  
...
* **Single Unified Document (`PAPER-001`):** The *JITC Meta-Framework* and the *Intuition Paper (Sept 5 2026)* represent one singular foundational manuscript (`paper_jitc_intuition.json`).
* **3-Section Anatomy:**
  - **Section 1: JITC Framework & Engineering Intuition** (synthesizes human intuition into system design; cites `WIS-xxx` and `PHL-xxx`).
...
* **`PHL-xxx` (Genuine Philosophy DNA):** High-level foundational mental models and epistemology extracted from *Philosophy and Learnings 2024–2026* and Google Keep notes (e.g. *Language as Humanity's Best Invention*, *The Vectors of Engineering*). The raw philosophy dumps are **not** the paper; they are reference sources for `PHL-DNA`.
...
* The writing workflow is explicitly split into three decoupled operational layers:
  1. **Structural Bones (`FEAT-581A` - Organization Tree View):** Drag-and-drop or slot lightweight citation chips (`[WIS-002]`, `[FEAT-104]`, `[ARXIV:2305.12345]`) into Section and Paragraph buckets.
  2. **Card Inspection (`FEAT-581B` - Single-Card Review Inspector):** Clicking any citation chip opens a clean sidebar displaying that card's origin quote, synthesis narrative, and anchors using the exact `wisdom.html` layout.
  3. **Prose Reflow (`FEAT-581C` - Synthesis View):** Hydrates paragraphs into continuous prose. Modifying citation associations in the tree marks the paragraph **`dirty`**, activating an actionable **[⚡ Revise / Re-synthesize]** button that triggers an LLM re-wordsmithing pass without touching adjacent paragraphs.
  4. **Revision History (`< >` Carousel):** Paragraphs track up to `3 revisions` with navigation arrows inside the inspector/toolbar to step back and forth or restore an earlier version.
...
#### 🧬 Story 78.1: Discrete Paper Dataset Storage (`[FEAT-580]`)
...
#### 🧬 Story 78.3: Writer Studio Multi-Paper UI & Review Inspector Panel (`[FEAT-580]` / `[FEAT-581B]`)
...
#### 🧬 Story 78.4: Foyer REST Paper API Endpoints (`[FEAT-580]`)
...
#### 🧬 Story 78.5: Structural Tree View & DNA Tag Drag-and-Drop (`[FEAT-581A]`) [RISK: MEDIUM]
...
#### 🧬 Story 78.6: Cascade Synthesis Reflow & 3-Tier Revision Carousel (`[FEAT-581C]` / `[FEAT-583]`) [RISK: HIGH]
