...
11. **Narrow Scope & On-Demand Reference Pattern**: To prevent massive codebase indexing (which swells baseline token counts to 40K+ and triggers API rate limit rejects), always initialize OpenAgent targeting the narrowest active subdirectory (e.g. `--dir /home/jallred/Dev_Lab/HomeLabAI`). Reference files outside this workspace scope (such as plans in `Portfolio_Dev`) using their absolute paths (`file://` scheme) in the prompt to allow the agent to fetch them on-demand via its read tools.
