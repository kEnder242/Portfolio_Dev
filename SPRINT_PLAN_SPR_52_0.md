...
### Task 53.7 — FEAT-437: `resolve_hyde_vector()` Kender HyDE Enrichment

**FEAT Anchor:** [`FeatureTracker.md:L1466`](file:///home/jallred/Dev_Lab/Portfolio_Dev/FeatureTracker.md#L1466) — `[FEAT-437] 3-Tier HyDE Failover Cascade & Vector Plumbing` — Status: ACTIVE (documented, unimplemented).

**Forensic Summary — Why Not Yet Done (confirmed 2026-08-12):**
- `FEAT-452` (Sprint 51, commit `14fc153`) delivered the 4-Domain HyDE Map: fast offline keyword gating in `_spawn_deep_thought_preamble()` at `router.py:1018`. This is Stage 1 of the 5-Stage DoL — it decides *whether* to HyDE but does NOT call Kender.
- The `hyde_vector_text` field consumed at `cognitive_hub.py:~1312` and passed to `get_context` at `~1319` is **currently generated by the local triage pre-reflection pass (z87 vLLM)** — that is Tier 2 operating as the de-facto sole path.
- Symbols `resolve_hyde_vector`, `DEEP_THOUGHT_REMOTE`, `PINKY_LOCAL_VLLM`, `DIRECT_RAW_QUERY` **do not exist anywhere in `HomeLabAI/src/`**. Forensically confirmed: `grep -rn "DEEP_THOUGHT_REMOTE\|resolve_hyde_vector" HomeLabAI/src/` → zero results.
- This story was not in the Sprint 52 task list by name. Recovered via FeatureTracker audit on 2026-08-12 session.

**Functional Requirement:**
Implement `resolve_hyde_vector(query, triage_result, timeout=8.0)` in `HomeLabAI/src/logic/cognitive_hub.py` that wraps the ChromaDB call in `process_query()` with a 3-tier fallback:

1. **`DEEP_THOUGHT_REMOTE` (Tier 1 — Kender 4090):** Call `thought.call_tool("deep_think", {"task": hyde_synthesis_prompt, "context": query})` via `asyncio.wait_for(..., timeout=timeout)`. On success, return the synthesized HyDE string. Log: `[FEAT-437][TIER1]`.
2. **`PINKY_LOCAL_VLLM` (Tier 2 — z87 vLLM, current live path):** Kender timed out or offline → use `hyde_vector_text` already in `triage_result` from the pre-reflection pass. Log: `[FEAT-437][TIER2]`.
3. **`DIRECT_RAW_QUERY` (Tier 3 — zero-dependency floor):** Both remote and local fail → pass raw `query` string directly as the ChromaDB vector. No crash. Log: `[FEAT-437][TIER3]`.
