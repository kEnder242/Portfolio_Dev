...
def main():
    ...
    # 5. Prompt Pinky
    # Determine if this is a fresh scan or refinement
    # For simplicity, we pass existing data and ask to MERGE/UPDATE.
    ...
    if isinstance(new_events, list):
        # Merge Logic
        # Naive merge: Append new events. 
        # Better merge: If similar summary exists, replace? 
        # Let's append unique ones for now.
        ...
        print(f"   > Saved {len(final_events)} events to {bucket_file} (+{added_count} new)")
        
        # 5b. Aggregate Year Data (YYYY_MM.json -> YYYY.json)
        # Extract year from bucket (YYYY-MM)
        if '-' in task['bucket']:
    ...
    # 6. Update State
    # Only if we successfully processed (or attempted), we mark state as updated
    # so we don't re-queue immediately.
    ...
    # Calculate hash of the content we just processed to mark it "Done"
    # Wait, the task had an ID. We need to store the hash of the *content* we processed.
    # The queue manager used md5(content).
    # We should update state[task['id']] = md5(task['content'])
    # Actually, scan_queue.py checks state.get(chunk_id) != content_hash.
    # So we must set state[chunk_id] = content_hash.
    ...
    # 7. Save Queue (Remove popped item)
