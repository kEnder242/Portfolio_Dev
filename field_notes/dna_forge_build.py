        ...
        (function () {{
            ...
            function runDecompose() {{
                ...
                .catch(function() {{
                    ...
                    var inferredTitle = title ? String(title).trim() : '';
                    if (!inferredTitle) {{
                        var matchTitle = cleanText.match(/^(?:#+\\s*|\\*\\*\\s*)([^\\n]+)/);
                        if (matchTitle) {{
                            inferredTitle = matchTitle[1].replace(/\\*\\*$/, '').trim();
                        }}
                    }}

                    var rawParagraphs = cleanText.split(/\\n\\s*\\n+/).map(function(p){{ return p.trim(); }}).filter(Boolean);
                    if (rawParagraphs.length <= 1) {{
                        rawParagraphs = cleanText.split(/\\n+(?=(?:#+|[A-Za-z0-9_\\-\\s]+:|[-*•]\\s+))/).map(function(p){{ return p.trim(); }}).filter(Boolean);
                    }}

                    // If first paragraph is purely the header line and we have subsequent paragraphs, extract title and advance
                    if (rawParagraphs.length > 1 && /^#+\\s+[^\\n]+$/.test(rawParagraphs[0])) {{
                        if (!inferredTitle) inferredTitle = rawParagraphs[0].replace(/^#+\\s*/, '').trim();
                        rawParagraphs.shift();
                    }}

                    var chunks = rawParagraphs.map(function(p, i) {{
                        var pLower = p.toLowerCase();
                        var dom = (pLower.indexOf('rule') !== -1 || pLower.indexOf('bkm') !== -1 || pLower.indexOf('mandate') !== -1) ? 'BKM' :
                                  (pLower.indexOf('axiom') !== -1 || pLower.indexOf('phl') !== -1 || pLower.indexOf('philosophy') !== -1) ? 'PHL' :
                                  (pLower.indexOf('feat') !== -1 || pLower.indexOf('feature') !== -1) ? 'FEAT' :
                                  (pLower.indexOf('rdna') !== -1 || pLower.indexOf('reverse') !== -1) ? 'RDNA' :
                                  (pLower.indexOf('disc') !== -1 || pLower.indexOf('discipline') !== -1) ? 'DISC' : 'WIS';
                        var lines = p.split('\\n').map(function(l){{ return l.trim(); }}).filter(Boolean);
                        var firstL = (lines[0] || '').replace(/^[#\\-\\*\•\\s]+/, '').replace(/^(?:Rule|Axiom|Philosophy|Guideline|Principle|Feature):\\s*/i, '').slice(0, 60);
                        return {{
                            chunk_id: 'CHUNK-' + (i + 1),
                            proposed_domain: dom,
                            ...
                            mutations: [
                                {{ id: 'mut_active', lens: 'Active Voice', text: 'Enforces ' + (firstL.toLowerCase() || 'rigorous execution') + ' with high operational discipline.' }}
                            ]
                        }};
                    }});

                    var fallbackData = {{
                        title: inferredTitle || 'Synthesis Note',
                        summary: 'Decomposed ' + chunks.length + ' chunks (client heuristic fallback).',
                        chunks: chunks,
                        suggested_bone_collection: {{
                            name: 'Track: ' + (inferredTitle || 'Synthesis Note'),
                            bones: chunks.map(function(c){{ return {{ id: c.chunk_id, title: c.title, domain: c.proposed_domain }}; }})
                        }}
                    }};
                    if (statusBadge) statusBadge.textContent = '✓ Decomposed ' + chunks.length + ' chunks (local heuristic fallback)';
