            ...
            function runDecompose() {{
                ...
                .then(function(data) {{
                    if (data.status === 'error') {{
                        throw new Error(data.message || 'Decomposition failed');
                    }}
                    if (statusBadge) statusBadge.textContent = '✓ Decomposed ' + (data.chunks ? data.chunks.length : 0) + ' pearls via live vLLM';
                    activeDecomposition = data;
                    renderDecompositionSandbox(data);
                }})
                .catch(function(err) {{
                    console.error('[DNA FORGE] Live Silicon Decompose Error:', err);
                    if (statusBadge) {{
                        statusBadge.innerHTML = '<span style="color:#f85149; font-weight:bold;">❌ Live Silicon Decompose Failed: ' + escapeHtml(String(err.message || err)) + '</span>';
                    }}
                    var sandbox = document.getElementById('decompositionSandbox');
                    alert('Live Silicon Decompose Failed (BKM-024 Mandate):\\n' + (err.message || err) + '\\n\\nEnsure Foyer (port 8765) and vLLM (port 8088) are online.');
                }});
            }}
            ...
            function initEgoSynapseCanvas() {{
                ...
                function computeConstellation() {{
                    ...
                    clusterIds.forEach(function(cid, idx) {{
                        targetSet[cid] = true;
                        var nData = globalNodesMap[cid] || cardsById[cid] || {{ id: cid, title: cid, domain: cid.split('-')[0] }};
                ...
                function stepPhysics() {{
                    ...
                    // 2. Multi-body Coulomb Repulsion (Ultra-Spacious & Breathable)
                    var kRep = 8500 * simEnergy;
                    ...
                    // 3. Spring Link Attraction (Expansive Rest Distance for Giant Cards)
                    var restDist = 320;
                    ...
                    nodes.forEach(function(n) {{
                        ...
                        n.vx += (cDx / cDist) * (cDist * 0.0004 * simEnergy);
                        n.vy += (cDy / cDist) * (cDist * 0.0004 * simEnergy);
                ...
                function draw() {{
                    ...
                    // --- [FEAT-603] Render Perimeter-Connected Synaptic Threads ---
                    activeLinks.forEach(function (l) {{
                        ...
                        // Compute clean perimeter boundary anchor points (cards calculate ray-box, dots connect to center)
                    ...
                    nodes.forEach(function (n) {{
                        ...
                        if (n.isGrandchild) {{
                            // Tier 3: Grandchildren are JUST DOTS (No grey boxes!)
                            ctx.beginPath();
                            ctx.arc(n.x, n.y, isHovered ? 8 : 6, 0, Math.PI * 2);
                            ctx.fillStyle = isHovered ? '#ffffff' : (n.color || '#6e7681');
                            if (isHovered) {{
                            ...
                            // Subtle ID label next to dot when hovered
                            if (isHovered) {{
                ...
                window.addEventListener('mousemove', function (e) {{
                    if (isPanning) {{
                        ...
                        // [FEAT-603] 1-Hop Hover Promotion Protocol
                        // 1. Center node hover does nothing (all info is already displayed on full canvas card)
                        // 2. Trail nodes promote up to FULL card in tooltip
                        // 3. Children nodes promote up to Tail node style in tooltip
                        // 4. Grandchildren dots promote up to Children node style (Name & Description)
                        if (hit && !hit.isCenter) {{
                            ...
                            if (hit.isTrail) {{
                                ...
                                // Promoted to TAIL NODE style view
                                var narrBlock = hit.narrative ? '<div style="color:#c9d1d9; font-size:0.76rem; line-height:1.4; margin-bottom:6px; background:rgba(0,0,0,0.4); border:1px solid #21262d; padding:6px 8px; border-radius:4px;">' + escapeHtml(hit.narrative) + '</div>' : '';
                                ...
                                // Promoted to CHILDREN NODE style view (Name & Description)
