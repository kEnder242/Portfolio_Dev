...
*   [x] **Task 16.1 (Llama Restoration)**: Verify and hard-code `start_vllm.sh` to use the proven Llama-3.2-3B-AWQ model. Smoke test vocal output and multi-LoRA stability.
*   [x] **Task 16.2 (Markdown Pop)**: Enable `marked.js` rendering for all Persona and Insight messages in `intercom_v2.js`.
*   [x] **Task 16.3 (Null-State Triage)**: Refactor `lab_node.py` to return empty `situation` and `hints` fields when `vibe == CASUAL`.
*   [x] **Task 16.4 (Analytical Quips)**: Implement a "loading quip" from the Brain/Archive node while Pinky is generating.
*   [x] **Task 16.5 (Signature Fix)**: Harden the WebSocket metadata payload in `router.py` to ensure the `version` field is always present.
*   [ ] **Task 16.6 (The Sanity Swap - VibeThinker/Qwen 3B)**: (Experimental) Re-point `start_vllm.sh` to evaluate VibeThinker-3B (huggingface.co/WeiboAI/VibeThinker-3B) as the local unified base, ensuring full Multi-LoRA support. Fallback: Qwen 3B. Revertible via `infrastructure.json`.
...
*Status: PLANNING | DRAFTED BY GEMINI CLI 3.1*
